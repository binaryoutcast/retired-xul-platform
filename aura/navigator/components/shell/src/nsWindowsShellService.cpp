...
//   are mapped like so:
//
//   HKCU\SOFTWARE\Classes\.<ext>\      (default)         REG_SZ     MarinerHTML
//
//   as aliases to the class:
//
//   HKCU\SOFTWARE\Classes\MarinerHTML\
...
//
//   HKCU\SOFTWARE\Classes\MarinerURL\  (default)         REG_SZ     <appname> URL
...
//   -------------------------------------------------
//   The following keys are set to make Mariner appear in the Start Menu as the
//   browser:
//   
//   HKCU\SOFTWARE\Clients\StartMenuInternet\Mariner.EXE\
...
// Mariner is the default browser for file handlers since other applications
...
// handler to another application and still have Mariner check if it is the
...
static SETTING gSettings[] = {
  ...
  { MAKE_KEY_NAME1("MarinerHTML", SOC), VAL_OPEN, OLD_VAL_OPEN },

  // Protocol Handler Class - for Vista and above
  { MAKE_KEY_NAME1("MarinerURL", SOC), VAL_OPEN, OLD_VAL_OPEN },
...
// since they are only checked when Mariner is the default browser and if they
// are incorrect they are fixed without notifying the user.
static SETTING gDDESettings[] = {
  // File Handler Class
  { MAKE_KEY_NAME1("Software\\Classes\\MarinerHTML", SOD) },

  // Protocol Handler Class - for Vista and above
  { MAKE_KEY_NAME1("Software\\Classes\\MarinerURL", SOD) },
             ...
             LPCWSTR aClassName)
{
  ...
  LPCWSTR progID = isProtocol ? L"MarinerURL" : L"MarinerHTML";
 ...
 * This only checks for MarinerURL and if aCheckAllTypes is set, then
 * it also checks for MarinerHTML.  Note that those ProgIDs are shared
 * by all Mariner browsers.
*/
bool
                                        ...
                                        bool* aIsDefaultBrowser)
{
  ...
  *aIsDefaultBrowser = true;
  ...
  // that was used to launch Mariner which is not necessarily a long path.
  ...
  for (; settings < end; ++settings) {
    ...
    if (REG_FAILED(res) ||
      ...
      if (REG_FAILED(res)) {
        ...
        // application when setting Mariner as the default browser.
        *aIsDefaultBrowser = false;
        return NS_OK;
      }
  ...
  // Only check if Mariner is the default browser on Vista and above if the
  // previous checks show that Mariner is the default browser.
  if (*aIsDefaultBrowser) {
  ...
  // account didn't perform a Mariner update this will check if Mariner is the
  // default browser and if dde is disabled for each handler
  // and if it isn't disable it. When Mariner is not the default browser the
  ...
  if (*aIsDefaultBrowser && aForAllTypes) {
    ...
    // update it using the helper application when setting Mariner as the
    // default browser.
    if (REG_FAILED(res)) {
      *aIsDefaultBrowser = false;
    }
                                            ...
                                            int32_t aPosition)
{
  ...
  // eventually, the path is "%APPDATA%\Mozilla\Mariner\Desktop Background.bmp"
